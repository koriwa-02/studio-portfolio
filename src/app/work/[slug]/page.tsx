...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> { const project = getLegacyProject((await params).slug); if (!project) return { title: `Project not found | ${SITE_NAME}` }; return { title: `${project.title} ${project.category} Case Study | ${SITE_NAME}`, description: project.overview, alternates: { canonical: absoluteUrl(`/projects/${project.slug}/`) } }; }
